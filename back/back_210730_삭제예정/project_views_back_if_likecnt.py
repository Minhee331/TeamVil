from teamvil.teamvil.project.models import Project
...
def like(request, project_id):
    like = Like()
    # user = request.user #'사용자' 인 나 가 user
    project = Project.objects.get(id=project_id) #아이디별로 프로젝트를 가져오기
    type = int(0)
    like.save()
    return render(request,"", {'project':project,'type':type})     
...
def likecnt_if(request, project_id):
    ...
    return render(request,"team_search_back_if.html", {'like_count':like_count})
#좋아요 카운팅 함수 / js버전
def likecnt(request, project_id):


# 팀찾기 최신순 정렬 함수 // 팀 모델에 register가 없어요
