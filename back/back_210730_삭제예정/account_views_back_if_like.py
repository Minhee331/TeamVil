...
def mypage_modify_profile_update(request):
    ...
    job = Job.objects.get(id=request.POST.get('profile.job.job'))
    profile.job_id = job
    profile.birthday = request.POST.get('birthday')
    region1s = Region1.objects.all()
    region1 = Region1.objects.get(id=request.POST.get('profile.region1.region1'))
    profile.region1_id = region1
    region2s = Region2.objects.all()
    region2 = Region2.objects.get(id=request.POST.get('profile.region2.region2'))
    ...
    term = Term.objects.get(id=request.POST.get('profile.term.term'))
    profile.term_id = term  
    mbtis = Mbti.objects.all()
    mbti = Mbti.objects.get(id=request.POST.get('profile.mbti.mbti'))
    ...
    user_files.save()
    user_carrers.save()
    return render(request,'/member/mypage_modify_profile_back_sunneng/update' + str(profile.id),{"region1s":region1s,"mbtis":mbtis,"terms":terms,"region2s":region2s,"jobs":jobs,"field1s":field1s})
...
def likecancel(request):
    ...
    to_user_id = Profile.objects.get(id=obj['value'])
    Like.objects.filter(to_user_id =to_user_id).delete() #like모델에 저장된 project_id랑 좋아요를 다시 클릭해서 얻어온 프로파일.user_id.id
    return render(request,'member_search_back.html')
