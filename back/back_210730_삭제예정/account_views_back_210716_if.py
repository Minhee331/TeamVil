...
def mypage_profile_back(request, user_id,to_user_id):
    profile = Profile.objects.get(id = user_id)
    carrers = User_carrer.objects.filter(user_id = profile.user_id)
    user_links = User_link.objects.filter(user_id = profile.user_id)
    user_files = User_file.objects.filter(user_id = profile.user_id)
    user_reviews = User_review.objects.filter(user_id =profile.user_id)
    return render(request, "mypage_profile_back.html", {"profile":profile, "carrers":carrers,
                "user_links": user_links, "user_files": user_files , "user_reviews": user_reviews})

def mypage_project_back(request,project_id): 
    project = Project.objects.filter(id=project_id)
    project = Profile.objects.get(id= request.user)
