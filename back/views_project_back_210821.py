...
def team_search(request):
    ...
    projects_reg = Project.objects.filter(field1_id = profile.field1_id).order_by('-view_cnt')[:8]
    ...
    return render(request, "team_search.html", {'projects':projects, "field1s":field1, "mbtis" : mbti, 
                                                    "region1s": region1s, "terms": term, "jobs": job, "projects_reg": projects_reg, "classify":classify})
...
def team_application(request, project_id):
    ...
    print(profiles)
    print(applications)
    return render(request, "team_application.html", {'applications':applications, 'profiles':profiles})
...
def classify_S(request):
    ...
    projects_reg = Project.objects.filter(type = int(0), field1_id = profile.field1_id).order_by('-view_cnt')[:8]
    field1 = Field1.objects.all() # 대분야 (ex IT)
    mbti = Mbti.objects.all()
    region2 = Region2.objects.all()
    region1 = Region1.objects.all() # ~시 (서울만 ~구)
    ...
    classify = 'S'
    return render(request, "team_search.html", {'projects':projects, "field1s":field1, "mbtis" : mbti, "region1s":region1,
                                                    "region2s": region2, "terms": term, "jobs": job, "projects_reg": projects_reg,"posts":posts, "classify":classify})
#대분류 함수(공모전)->리로드
def classify_C(request):
    user = request.user
    profile = Profile.objects.get(user_id = user)
    projects = Project.objects.filter(type = int(1))
    projects_reg = Project.objects.filter(type = int(1), field1_id = profile.field1_id).order_by('-view_cnt')[:8]
    field1 = Field1.objects.all() # 대분야 (ex IT)
    mbti = Mbti.objects.all()
    region2 = Region2.objects.all()
    region1 = Region1.objects.all() # ~시 (서울만 ~구)
...
def classify_P(request):
    ...
    projects_reg = Project.objects.filter(type = int(2), field1_id = profile.field1_id).order_by('-view_cnt')[:8]
    field1 = Field1.objects.all() # 대분야 (ex IT)
    mbti = Mbti.objects.all()
    region2 = Region2.objects.all()
    region1 = Region1.objects.all() # ~시 (서울만 ~구)
